...
public class ConciliacionOperacionesUpdateEstadoController {
   ...
   * @return Un List de operaciones porgramadas indicando si se actualizo correctamente por cada
   *         operacion programada
   */
  @PostMapping(value = "${endpoints.conciliacion.update-estado-programadas}",
      consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<ApiResponseADE<List<UpdateEstadoOperacionesDTO>>> updateEstadoProgramadas(
      @RequestBody(
          required = true) List<UpdateEstadoOperacionesDTO> updateEstadoProgramadasDTOList) {
    log.info("Operaciones programadas para actualizar estado: {}",
        updateEstadoProgramadasDTOList.size());
    ...
    long exitosas = respuesta.stream().filter(operacion -> operacion.isUpdateExitoso()).count();
    long noExitosas = respuesta.stream()
        .filter(operacion -> Boolean.FALSE.equals(operacion.isUpdateExitoso())).count();
    return ResponseEntity.status(HttpStatus.OK)
        .body(new ApiResponseADE<>(respuesta,
            ResponseADE.builder().code(ApiResponseCode.SUCCESS.getCode())
                .description(ApiResponseCode.SUCCESS.getDescription() + ": Actualizacion de operaciones Exitosas: "
                    + exitosas + " - no exitosas: " + noExitosas)
                .build()));
  }
   ...
   * @return Un List de operaciones certificadas indicando si se actualizo correctamente por cada
   *         operacion certificada
   */
  @PostMapping(value = "${endpoints.conciliacion.update-estado-certificadas}",
      consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<ApiResponseADE<List<UpdateEstadoOperacionesDTO>>> updateEstadoCertificadas(
      @RequestBody(
          required = true) List<UpdateEstadoOperacionesDTO> updateEstadoCertificadasDTOList) {
    log.info("Operaciones certificadas para actualizar estado: {}",
        updateEstadoCertificadasDTOList.size());
